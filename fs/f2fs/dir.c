...
bool f2fs_fill_dentries(struct file *file, struct f2fs_dentry_ptr *d,
			unsigned int start_pos, void *dirent, filldir_t filldir)
{
	...
	while (bit_pos < d->max) {
		...
		if (de->file_type < F2FS_FT_MAX)
			...
			d_type = DT_UNKNOWN;
		if (!__dir_emit(filldir, file, d->filename[bit_pos],
...
static int f2fs_readdir(struct file *file, void *dirent, filldir_t filldir)
{
	...
	for (; n < npages; n++) {
		...
		if (f2fs_fill_dentries(file, &d, n * NR_DENTRY_IN_BLOCK, dirent, filldir))
			goto stop;
